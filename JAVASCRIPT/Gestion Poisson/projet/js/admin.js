...
function rechargerSection(section) {
  ...
  contenu.style.transform = "translateY(-8px)";
  contenu.style.transition = "opacity 0.18s ease, transform 0.18s ease";
  ...
  setTimeout(() => {
    switch (section) {
      ...
      case "poisson":
        ...
        getProduits().then(produits => { contenu.innerHTML = getSectionPoisson(produits); appliquerTransition(contenu); });
        break;
      case "stock":
        contenu.innerHTML = "<p>Chargement...</p>";
        Promise.all([getProduits(), getTransactions(), getStocks()]).then(([produits, transactions, stocks]) => {
          contenu.innerHTML = getSectionStock(produits, transactions, stocks); appliquerTransition(contenu);
        });
        break;
      case "vente":
        contenu.innerHTML = "<p>Chargement...</p>";
      getProduits().then(produits => { contenu.innerHTML = getSectionVente(produits); appliquerTransition(contenu); });
        break;
      case "commandes":
    ...
    appliquerTransition(contenu);
  }, 60);
}

function appliquerTransition(contenu) {
  contenu.style.transition = "none";
  contenu.style.opacity = "0";
  contenu.style.transform = "translateY(10px)";
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      contenu.style.transition = "opacity 0.28s ease, transform 0.28s ease";
...
function getSectionStock(produits, transactions, stocks) {
  ...
  return `
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-bottom:24px;">
      <div class="metrique-stock" style="background:#1565c0;color:black;"><div style="font-size:12px;opacity:.8;">Stock total</div><div style="font-size:24px;font-weight:bold;">${stockTotal.toLocaleString()} kg</div></div>
      <div class="metrique-stock" style="background:#ff9800;color:black;"><div style="font-size:12px;opacity:.8;">Produit en stock</div><div style="font-size:24px;font-weight:bold;">${enStock}</div></div>
      <div class="metrique-stock" style="background:#9c27b0;color:black;"><div style="font-size:12px;opacity:.8;">Hors stock</div><div style="font-size:24px;font-weight:bold;">${horsStock}</div></div>
      <div class="metrique-stock" style="background:#f44336;color:black;"><div style="font-size:12px;opacity:.8;">Rupture de stock</div><div style="font-size:24px;font-weight:bold;">${ruptureStock}</div></div>
    </div>
    <div class="section-tableau">
